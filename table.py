...
class BoardUI:
    ...
    # TODO read file and complete action for UI @mohamadnikan2013@gmail.com
    def draw_board(self):
        ...
        for i in range(dimension ** 2):
            if i % dimension == 0:
                self.y_coord -= self.side
                my_pen.penup()
                my_pen.setpos(self.x_coord, self.y_coord)
                my_pen.pendown()
        ...
        for i in range(1, 6):
            for j in range(5):
                lalist.append([myxcoord + (j * self.side), myycoord - (self.side * i)])
        my_pen.penup()
        index = 0
        for i in lalist:
    ...
    def draw_down_condition(self, x):
        my_pen = turtle.Turtle()
        my_pen.speed('fastest')
        my_pen.hideturtle()
        my_pen.pensize(3)
    ...
    def draw_left_condition(self, x):
        my_pen = turtle.Turtle()
        my_pen.speed('fastest')
        my_pen.hideturtle()
        my_pen.pensize(3)
    ...
    def draw_right_condition(self, x):
        my_pen = turtle.Turtle()
        my_pen.speed('fastest')
        my_pen.hideturtle()
        my_pen.pensize(3)
